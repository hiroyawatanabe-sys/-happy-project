/* =============================================================
   新・七つの大罪 診断ツール（seven-cta）東宣版
   ※ガーディアン版 js/seven-cta.js のコピー。差分は表示文字列と
     データ文字列のみ（判定ロジック・診断機構は完全に同一）。
...
(function () {
  ...
  ----------------------------------------------------------- */
  var INDUSTRIES = {
    '住宅・リフォーム': ['工務店・注文住宅', 'リフォーム・リノベーション', '不動産売買・仲介', 'エクステリア・造園', '賃貸管理'],
    '自動車': ['自動車販売（新車・中古車）', '整備・車検', 'コーティング・用品'],
    '農業・食品': ['食品メーカー', '農業資材・農薬', 'JA・団体', '食品卸・小売'],
    '士業・コンサルティング': ['税理士・会計士', '弁護士・司法書士', '社労士・行政書士', '経営コンサルティング'],
    '医療・福祉': ['クリニック', '歯科', '整骨院・鍼灸', '介護・福祉施設'],
    '美容・健康': ['美容室', 'エステ・脱毛', 'フィットネス', '健康食品'],
    '飲食・食品店': ['レストラン・食堂', 'カフェ', '居酒屋・バー', 'テイクアウト・通販'],
    '小売・EC': ['店舗小売', 'ECサイト', '卸売'],
    '教育・スクール': ['学習塾・予備校', '習い事教室', '資格スクール'],
    '冠婚葬祭・生活サービス': ['葬儀・霊園', 'ブライダル', '清掃・ハウスクリーニング', '旅行・宿泊'],
    '製造・工業': ['金属・機械加工', '印刷', '電子・電気', 'その他製造'],
    'その他': ['その他']
  };
  ...
  function checkRateLimit() {
    ...
    try {
      var now = Date.now();
      var log = JSON.parse(localStorage.getItem('tosenDiagLog') || '[]')
        .filter(function (t) { return now - t < RATE_LIMIT.windowMs; });
      if (log.length >= RATE_LIMIT.max) return false;
      log.push(now);
      localStorage.setItem('tosenDiagLog', JSON.stringify(log));
      return true;
    } catch (e) {
      return true;
    }
  ...
  function _startDiagnosis(trigger) {
    ...
    runDiagnosis(value)
      .then(function (results) {
        ...
        state.running = false;
        el.status.textContent = 'すべてのチェックが完了しました。';
  ...
  function animateResults(results) {
    ...
    results.forEach(function (result, idx) {
      ...
      chain = chain.then(function () {
        ...
        el.status.textContent = cat.roman + '. ' + cat.name + ' をチェック中…';
  ...
  function _submitSurvey(event) {
    ...
    try { localStorage.setItem('tosenDiagSurvey', JSON.stringify(payload)); } catch (e) { /* 保存不可でも続行 */ }
  ...
  function overallGrade() {
    ...
    if (fails === 0 && warns <= 1) return { label: '小', cls: '-w-grade-excellent' };
    if (fails <= 1) return { label: '中', cls: '-w-grade-caution' };
    if (fails <= 3) return { label: '大', cls: '-w-grade-improve' };
    return { label: '特大', cls: '-w-grade-danger' };
  }
  ...
  function buildReport() {
    ...
    el.reportContainer.innerHTML =
      '<article class="-w-report-card" aria-label="WEB集客 取りこぼし診断 成績表">' +
        '<div class="-w-report-head">' +
          '<div>' +
            '<h2 class="-w-report-title"><span>WEB集客 取りこぼし診断</span> 成績表</h2>' +
            '<p class="-w-report-meta">診断URL：' + escapeHtml(state.url) + '<br>' +
              '診断日時：' + stamp + '　合格項目：' + totalPass + ' / 21</p>' +
          '</div>' +
          '<div class="-w-report-stamp ' + grade.cls + '" role="img" aria-label="取りこぼしリスク：' + grade.label + '">' +
            '<span class="-w-report-stamp-label">取りこぼし<br>リスク</span>' +
        ...
        '<p class="-w-report-note">※本診断は公開情報の自動解析による簡易診断であり、結果は推定を含みます。実際の集客状況・改善効果を保証するものではありません。</p>' +
      '</article>';
  }
